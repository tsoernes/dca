...
from nets.utils import build_default_trainer, get_trainable_vars, prep_data_grids
...
class TDCNLSinghNet(Net):
    ...
    def _build_net(self, top_inps):
        with tf.variable_scope('model/' + self.name) as scope:
            ...
            vals = [
                value_layer.apply(tf.layers.flatten(conv2a.apply(conv1a.apply(top_inps[0])))),
                value_layer.apply(tf.layers.flatten(conv2b.apply(conv1b.apply(top_inps[1]))))
            ]
