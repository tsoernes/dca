    ...
    def V(self, theta=None):
        ...
        if theta is None:
            if not hasattr(self, "theta"):
                raise Exception("no theta available, has to be specified" " by parameter")
            theta = self.theta
...
class GeriTDC(TDC):
    """
    the TDC algorithm except that the pseudo-stationary
    guess for off-policy estimation is computed differently
    """
...
class KTD(LinearValueFunctionPredictor):
    ...
    def update_V(self, s0, s1, r, f0=None, f1=None, theta=None, rho=1, **kwargs):
        ...
        Pn[:-2, :-2] += self.P_vi
        Pn[-2:, -2:] += np.array([[1., -self.gamma], [-self.gamma, self.gamma**2]
                                  ]) * self.reward_noise

        # Compute Sigma Points
        X, W = self.sample_sigma_points(xn, Pn)
        R = (np.dot(f0, X[:, :-2].T) - self.gamma * np.dot(f1, X[:, :-2].T) +
             X[:, -1].T).flatten()
...
class GPTDP(LinearValueFunctionPredictor):
    ...
    Parametric GPTD
    for details see
     Engel, Y. (2005). Algorithms and Representations for
    Reinforcement Learning. Hebrew University.
    Algorithm 18
    """
    ...
    def update_V(self, s0, s1, r, f0=None, f1=None, theta=None, rho=1, **kwargs):
        ...
        s = self.sigma ** 2 + self.gamma ** 2 * self.sigma ** 2 - \
            self.sigma ** 2 * self.gamma * c + \
...
class GPTD(ValueFunctionPredictor):
    ...
    def update_V(self, s0, s1, r, f0=None, f1=None, theta=None, rho=1, **kwargs):
        ...
        if delta > self.nu:
            ...
            dk2 = np.array((self.kernel(self.D, s0) -
                            2 * self.gamma * self.kernel(self.D, s1))).flatten()
            self.D.append(s1)
            # update K^-1
            self.Kinv.view = delta * self.Kinv.view + np.outer(a, a)
            self.Kinv.expand(
                cols=-a.reshape(-1, 1), rows=-a.reshape(1, -1), block=np.array([[1]]))
            ...
            self.c.view = self.c.view.flatten(
            ) * self.sinv * self.gamma * self.sigma0**2 + self.a - np.dot(
                self.C.view, dk)
            self.c.expand(rows=np.array(-self.gamma))

            s = (1 + self.gamma ** 2) * self.sigma0 ** 2 - \
                self.sinv * self.gamma ** 2 * self.sigma0 ** 4 + dktt - \
            ...
            self.C.expand(
                rows=np.zeros((1, self.C.shape[1])), cols=np.zeros((self.C.shape[0], 1)))
            ...
            self.c.view = self.c.view.flatten(
            ) * self.sinv * self.gamma * self.sigma0**2 + self.hbar - np.dot(
                self.C.view, dk)

            s = (1 + self.gamma ** 2) * self.sigma0 ** 2 - \
                self.sinv * self.gamma ** 2 * self.sigma0 ** 4 + \
...
class BRMDS(BRM):
    def update_V(self,
        ...
        self.b = (1 - alpha) * self.b + alpha * (
            df * rho * rhot * rt + dft * rho * rhot * r) / 2.
