...
from baselines.acktr.utils import (Scheduler, cat_entropy, discount_with_dones,
...
def learn(policy,
    ...
    make_model = lambda: Model(policy, ob_space, ac_space, nenvs, total_timesteps,
                               nprocs=nprocs, nsteps=nsteps, nstack=nstack, ent_coef=ent_coef,
                               vf_coef=vf_coef, vf_fisher_coef=vf_fisher_coef, lr=lr,
                               max_grad_norm=max_grad_norm, kfac_clip=kfac_clip,
    ...
    for update in range(1, total_timesteps // nbatch + 1):
        ...
        policy_loss, value_loss, policy_entropy = model.train(obs, states, rewards, masks,
                                                              actions, values)
        model.old_obs = obs
