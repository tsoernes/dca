...
from baselines.acktr.kfac_utils import detectMinVal, factorReshape, gmatmul
...
class KfacOptimizer():
        ...
        def searchFactors(gradient, graph):
            ...
            if 'AddN' in bpropOp_name:
                ...
                if len(factors[0]['fpropFactors']) > 0:
                    fTensors = reduce(lambda x, y: x + y,
                                      [item['fpropFactors'] for item in factors])
                fpropOp_name = op_names[0]
                fpropOp = factors[0]['op']
            else:
                fpropOp_name = re.search('gradientsSampled(_[0-9]+|)/(.+?)_grad',
                ...
                if fpropOp.op_def.name in KFAC_OPS:
                    ...
                    bInputsList = [
                        i for i in bpropOp.inputs[0].op.inputs if 'gradientsSampled' in i.name
                ...
                for item in varlist:
                    if len(factorTensors[item]['bpropFactors']) > 0:
                        if (set(factorTensors[item]['bpropFactors']) == set(
                                factorTensors[param]['bpropFactors'])) and (len(
                                    factorTensors[item]['fpropFactors']) > 0):
            ...
            for i, param in enumerate(varlist):
                ...
                for j, param2 in enumerate(varlist[(i + 1):]):
                    if (len(factorTensors[param][key]) > 0) and (set(
                            factorTensors[param2][key]) == set(factorTensors[param][key])):
                        factorTensors[param2][key] = factorTensors[param][key]
                        factorTensors[param2][key + '_concat'] = factorTensors[param][key
                                                                                      + '_concat']
        ########
        ...
        for var in varlist:
            ...
            for stats_var in fpropStats_vars:
                ...
                if stats_var not in statsUpdates_cache:
                    ...
                    if opType == 'Conv2D':
                        ...
                        if Oh == 1 and Ow == 1 and self._channel_fac:
                            ...
                            if len(SVD_factors) == 0:
                                if KFAC_DEBUG:
                                    print(('approx %s act factor with rank-1 SVD factors' %
                                           (var.name)))
                    ...
                    # divided by B
                    cov = tf.matmul(
                        fpropFactor, fpropFactor, transpose_a=True) / tf.cast(B, tf.float32)
            ...
            for stats_var in bpropStats_vars:
                ...
                if stats_var not in statsUpdates_cache:
                    ...
                    if opType == 'Conv2D' or len(bpropFactor_shape) == 4:
                        if fpropFactor is not None:
                            if self._approxT2:
                                ...
                                bpropFactor = tf.reduce_sum(bpropFactor,
                                                            [1, 2])  # T^2 terms * 1/T^2
                            else:
                                bpropFactor = tf.reshape(bpropFactor,
                    ...
                    cov_b = tf.matmul(
                        bpropFactor, bpropFactor, transpose_a=True) / tf.to_float(
        ...
        def updateAccumStats():
            if self._full_stats_init:
                return tf.cond(
                    ...
                    lambda: tf.group(
                        *self._apply_stats(
                            statsUpdates, accumulate=True, accumulateCoeff=1. /
                            self._stats_accum_iter)), tf.no_op)
            else:
                return tf.group(*self._apply_stats(
                    statsUpdates, accumulate=True, accumulateCoeff=1. / self._stats_accum_iter))
    ...
    def _apply_stats(self, statsUpdates, accumulate=False, accumulateCoeff=0.):
        ...
        if KFAC_DEBUG:
            stats_step_op = (tf.Print(stats_step_op, [
                ...
                tf.convert_to_tensor('stat step:'), self.stats_step, updateOps[0], updateOps[1]
            ]))
        return [
            stats_step_op,
        ]
    ...
    def computeStatsEigen(self):
        """ compute the eigen decomp using copied var
        stats to avoid concurrent read/write from other queue """
    ...
    def getKfacPrecondUpdates(self, gradlist, varlist):
        ...
        for grad, var in zip(gradlist, varlist):
            ...
            if (len(fpropFactoredFishers) + len(bpropFactoredFishers)) > 0:
                ...
                # grad = tf.Print(
                #   grad, [tf.convert_to_tensor('2'), tf.convert_to_tensor(var.name), grad.get_shape()])
                #####
        ...
        with tf.control_dependencies([global_step_op]):
            ...
            with tf.control_dependencies(dependency_list):
                ...
                with tf.control_dependencies([updateFactorOps]):
                    ...
                    u = tf.cond(
                        tf.greater(self.factor_step, tf.convert_to_tensor(0)), getKfacGradOp,
                        gradOp)

                    optim = tf.train.MomentumOptimizer(self._lr * (1. - self._momentum),
                                                       self._momentum)
                    ...
                    def optimOp():
                        ...
                        if self._full_stats_init:
                            ...
                            return tf.cond(
                                tf.greater_equal(self.sgd_step, self._cold_iter), updateOptimOp,
                                tf.no_op)
